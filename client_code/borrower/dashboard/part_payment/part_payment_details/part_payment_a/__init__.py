...
class part_payment_a(part_payment_aTemplate):
  def __init__(self, selected_row, total_payments_made, **properties):
    ...
    monthly_interest_rate = selected_row["interest_rate"] / (
      12 * 100
    )  # Assuming interest rate is in percentage
    factor = (
      1 + monthly_interest_rate
    ) ** tenure  # Calculate (1 + r)^t without using pow
  ...
  def button_2_click(self, **event_args):
    """This method is called when the button is clicked"""
    open_form(
      "borrower.dashboard.foreclosure_request.borrower_foreclosure",
  ...
  def link_1_click(self, **event_args):
    ...
    alert(
      "Submitting a foreclosure request implies acknowledgment and acceptance of financial and legal consequences, adhering to established timelines and communication protocols."
    )
